...
<CodeBlock client:load dependencies={{ '@aracna/core': 'latest' }} files={SB_ARACNA_BLOB_BASE64} template='javascript' title='aracna-blob' console={100} />
...
<Alert client:load className='mb-6' severity='info'>
  Note that the resolveArrayBuffer method must be called on the deserialized AracnaBlob object to resolve the ArrayBuffer before accessing the uInt8Array
  property.
</Alert>
