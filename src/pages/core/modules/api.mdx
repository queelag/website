...
The **API** class provides a convenient way to make HTTP requests to a server with TypeScript. By using the **API** class, you can handle different types of HTTP requests and configuration options in a concise and readable way.

## Isomorphic

The **API** class is isomorphic because it uses the **Polyfill** class to polyfill the **fetch**, **Blob**, **File**, and **FormData** APIs. This allows the **API** class to work in both server-side and client-side environments, even if those environments do not support these APIs natively.
...
<StackBlitz
  client:load
  dependencies={{ '@aracna/core': 'latest' }}
  files={SB_API_TRANSFORM_QUERY_PARAMETERS}
  template='javascript'
  title='API'
  console={100}
/>
